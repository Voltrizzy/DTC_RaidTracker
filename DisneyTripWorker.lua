-- 1. INITIALIZATION
local DTC_VERSION = "4.2.0" -- Read-Only Config & RCLC History
...
function DTC_CreateUI()
    ...
    frame.finalizeBtn:SetScript("OnClick", function()
        local raidName = GetInstanceInfo(); local dStr = date("%Y-%m-%d")
        for p, v in pairs(currentVotes) do
            if not p:find("_VOTED_BY_ME") and v > 0 then C_ChatInfo.SendAddonMessage(DTC_PREFIX, "FINALIZE:"..p..","..v..","..lastBossName..","..raidName..","..dStr, "RAID") end
        end
    ...
    frame.announceBtn:SetScript("OnClick", function()
        ...
        for i=1, math.min(3, #sorted) do local dispName = sorted[i].n; local nick = DTCRaidDB.identities[dispName]; if nick then dispName = dispName .. " ("..nick..")" end; SendChatMessage(i .. ". " .. dispName .. " (" .. sorted[i].v .. " pts)", "RAID") end
    end)
...
function DTC_RefreshVotingList()
    ...
    for i = 1, GetNumGroupMembers() do
        ...
        if name then
            ...
            local nameTxt = row:CreateFontString(nil, "OVERLAY", "GameFontHighlight"); nameTxt:SetPoint("LEFT", 5, 0); nameTxt:SetText(name)
...
-- 5. OPTIONS PANEL (Integrated & Role Aware)
function DTC_InitOptionsPanel()
    local panel = CreateFrame("Frame", "DTC_OptionsPanel", UIParent); panel.name = "DTC Raid Tracker"
    local title = panel:CreateFontString(nil, "ARTWORK", "GameFontNormalLarge"); title:SetPoint("TOPLEFT", 16, -16); title:SetText("DTC Raid Tracker: Nickname Configuration")
    local sub = panel:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall"); sub:SetPoint("TOPLEFT", title, "BOTTOMLEFT", 0, -8); sub:SetText("Assign nicknames. Leader can edit; others can view.")
    local sf = CreateFrame("ScrollFrame", "DTC_ConfigScroll", panel, "UIPanelScrollFrameTemplate"); sf:SetPoint("TOPLEFT", 16, -50); sf:SetPoint("BOTTOMRIGHT", -30, 16)
    local content = CreateFrame("Frame", nil, sf); content:SetSize(600, 1); sf:SetScrollChild(content); panel.content = content
    panel:SetScript("OnShow", function() DTC_RefreshOptionsList(panel.content) end)
    local category = Settings.RegisterCanvasLayoutCategory(panel, "DTC Raid Tracker")
    Settings.RegisterAddOnCategory(category)
    DTC_OptionsCategoryID = category:GetID()
end

function DTC_RefreshOptionsList(content)
    if not content then return end
    for _, child in ipairs({content:GetChildren()}) do child:Hide(); child:SetParent(nil) end
    
    -- Permission Check: Leader OR Solo (not in group) can edit
    local canEdit = UnitIsGroupLeader("player") or (not IsInGroup())
    
    local keys = {}
    for k, _ in pairs(DTCRaidDB.identities) do table.insert(keys, k) end
    -- Also ensure current group is in list even if no nick
    for i = 1, GetNumGroupMembers() do 
        local name = GetRaidRosterInfo(i)
    ...
    -- Remove duplicates in keys list from merge
    ...
    for _, name in ipairs(uniqueKeys) do
        local row = CreateFrame("Frame", nil, content); row:SetSize(500, 24); row:SetPoint("TOPLEFT", 0, yOffset)
        local label = row:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
        label:SetPoint("LEFT", 5, 0); label:SetWidth(150); label:SetJustifyH("LEFT"); label:SetText(name)
        ...
        eb:SetEnabled(canEdit) -- Enforce Permission
...
function DTC_CreateHistoryUI()
    ...
    hf:SetScript("OnDragStop", function(self) self:StopMovingOrSizing(); DTCRaidDB.settings.histPos = {self:GetPoint()} end)
    ...
    -- Resizer
    local resizer = CreateFrame("Button", nil, hf)
    resizer:SetSize(16, 16); resizer:SetPoint("BOTTOMRIGHT", -10, 10)
    resizer:SetNormalTexture("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Up")
    resizer:SetScript("OnMouseDown", function() hf:StartSizing("BOTTOMRIGHT") end)
    resizer:SetScript("OnMouseUp", function() hf:StopMovingOrSizing(); DTCRaidDB.settings.histSize = {hf:GetWidth(), hf:GetHeight()} end)

    -- Headers
    local hTime = hf:CreateFontString(nil, "OVERLAY", "GameFontHighlight"); hTime:SetPoint("TOPLEFT", 20, -50); hTime:SetText("Time")
    local hRaid = hf:CreateFontString(nil, "OVERLAY", "GameFontHighlight"); hRaid:SetPoint("LEFT", hTime, "RIGHT", 80, 0); hRaid:SetText("Raid / Boss")
    local hChamp = hf:CreateFontString(nil, "OVERLAY", "GameFontHighlight"); hChamp:SetPoint("LEFT", hRaid, "RIGHT", 180, 0); hChamp:SetText("Champion")
    local hVote = hf:CreateFontString(nil, "OVERLAY", "GameFontHighlight"); hVote:SetPoint("LEFT", hChamp, "RIGHT", 100, 0); hVote:SetText("Voters")

    -- Scroll
    local sf = CreateFrame("ScrollFrame", nil, hf, "UIPanelScrollFrameTemplate")
    sf:SetPoint("TOPLEFT", 20, -70); sf:SetPoint("BOTTOMRIGHT", -30, 50)
    local content = CreateFrame("Frame", nil, sf); content:SetSize(800, 1); sf:SetScrollChild(content)
    hf.content = content

    -- Buttons
    local closeBtn = CreateFrame("Button", nil, hf, "UIPanelButtonTemplate")
    closeBtn:SetSize(80, 25); closeBtn:SetPoint("BOTTOMRIGHT", -20, 15); closeBtn:SetText("Close")
    closeBtn:SetScript("OnClick", function() hf:Hide() end)

    local exportBtn = CreateFrame("Button", nil, hf, "UIPanelButtonTemplate")
    exportBtn:SetSize(100, 25); exportBtn:SetPoint("RIGHT", closeBtn, "LEFT", -10, 0); exportBtn:SetText("Export CSV")
    exportBtn:SetScript("OnClick", function()
        local exportBuffer = { "Date,Raid,Boss,Champion,Points,Voters" }
        for _, h in ipairs(DTCRaidDB.history) do table.insert(exportBuffer, string.format("%s,%s,%s,%s,%d,%s", h.d, h.r, h.b, h.w, h.p, h.v or "")) end
        local str = table.concat(exportBuffer, "\n")
        local eb = CreateFrame("EditBox", nil, hf, "InputBoxTemplate"); eb:SetSize(600, 30); eb:SetPoint("BOTTOM", 0, -10); eb:SetText(str); eb:HighlightText(); eb:SetFocus()
    end)
...
function DTC_RefreshHistory()
    ...
    for _, h in ipairs(DTCRaidDB.history) do
        ...
        local tDate = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall"); tDate:SetPoint("LEFT", 0, 0); tDate:SetWidth(100); tDate:SetJustifyH("LEFT"); tDate:SetText(h.d)
        local tBoss = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall"); tBoss:SetPoint("LEFT", 110, 0); tBoss:SetWidth(180); tBoss:SetJustifyH("LEFT"); tBoss:SetText(h.b)
        
        -- Champion Name (Apply Nickname if exists)
        local champName = h.w
        if DTCRaidDB.identities[h.w] then champName = h.w .. " ("..DTCRaidDB.identities[h.w]..")" end
        local tChamp = row:CreateFontString(nil, "OVERLAY", "GameFontNormal"); tChamp:SetPoint("LEFT", 300, 0); tChamp:SetWidth(120); tChamp:SetJustifyH("LEFT"); tChamp:SetText(champName)
        
        local tVoters = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall"); tVoters:SetPoint("LEFT", 430, 0); tVoters:SetJustifyH("LEFT"); tVoters:SetText(h.v or "None")
        
        yOffset = yOffset - 20
    end
end

-- 7. LEADERBOARD UI (EXISTING)
...
function DTC_CreateLeaderboardUI()
    ...
    lb.syncBtn = CreateFrame("Button", nil, lb, "UIPanelButtonTemplate"); lb.syncBtn:SetSize(80, 22); lb.syncBtn:SetPoint("BOTTOMLEFT", 15, 15); lb.syncBtn:SetText("Sync All"); lb.syncBtn:SetScript("OnClick", function() if UnitIsGroupLeader("player") then DTC_BroadcastFullSync() end end)
    lb.verBtn = CreateFrame("Button", nil, lb, "UIPanelButtonTemplate"); lb.verBtn:SetSize(90, 22); lb.verBtn:SetPoint("BOTTOMLEFT", 100, 15); lb.verBtn:SetText("Ver Check"); lb.verBtn:SetScript("OnClick", function() if UnitIsGroupLeader("player") then C_ChatInfo.SendAddonMessage(DTC_PREFIX, "VER_QUERY", "RAID") end end)
    lb.configBtn = CreateFrame("Button", nil, lb, "UIPanelButtonTemplate"); lb.configBtn:SetSize(90, 22); lb.configBtn:SetPoint("BOTTOMLEFT", 195, 15); lb.configBtn:SetText("Config IDs"); lb.configBtn:SetScript("OnClick", function() if Settings and Settings.OpenToCategory then Settings.OpenToCategory(DTC_OptionsCategoryID) else InterfaceOptionsFrame_OpenToCategory("DTC Raid Tracker") end end)
    lb.announceBtn = CreateFrame("Button", nil, lb, "UIPanelButtonTemplate"); lb.announceBtn:SetSize(90, 22); lb.announceBtn:SetPoint("BOTTOMLEFT", 290, 15); lb.announceBtn:SetText("Announce"); lb.announceBtn:SetScript("OnClick", function() local data = DTC_GetSortedData(); local t = (selBoss~="ALL" and selBoss) or (selRaid~="ALL" and selRaid) or (selExp~="ALL" and EXPANSION_NAMES[tonumber(selExp)]) or "All Time"; if selTime == "TODAY" then t = t .. " (Today)" end; SendChatMessage("--- DTC: " .. t .. " [" .. viewMode .. "] ---", "RAID"); for i=1, math.min(10, #data) do SendChatMessage(i .. ". " .. data[i].n .. ": " .. data[i].v, "RAID") end end)
    lb.awardBtn = CreateFrame("Button", nil, lb, "UIPanelButtonTemplate"); lb.awardBtn:SetSize(100, 22); lb.awardBtn:SetPoint("LEFT", lb.announceBtn, "RIGHT", 5, 0); lb.awardBtn:SetText("Award Trip"); lb.awardBtn:SetScript("OnClick", function() local data = DTC_GetSortedData(); if #data > 0 then local winnerName = data[1].n; DTCRaidDB.trips[winnerName] = (DTCRaidDB.trips[winnerName] or 0) + 1; SendChatMessage("--- DTC DISNEY TRIP AWARD ---", "RAID"); SendChatMessage("CONGRATULATIONS " .. winnerName .. "! You have won a trip to Disney!", "RAID"); C_ChatInfo.SendAddonMessage(DTC_PREFIX, "SYNC_DATA:TRIP,"..winnerName..","..DTCRaidDB.trips[winnerName], "RAID"); DTC_RefreshLeaderboard() else print("|cFFFF0000DTC:|r No votes found to award.") end end)
    ...
    lb.selfResetBtn = CreateFrame("Button", nil, lb, "UIPanelButtonTemplate"); lb.selfResetBtn:SetSize(80, 22); lb.selfResetBtn:SetPoint("RIGHT", lb.closeBtn, "LEFT", -10, 0); lb.selfResetBtn:SetText("Self Reset"); lb.selfResetBtn:SetScript("OnClick", function() StaticPopup_Show("DTC_SELF_RESET_CONFIRM") end)
    lb.resetBtn = CreateFrame("Button", nil, lb, "UIPanelButtonTemplate"); lb.resetBtn:SetSize(80, 22); lb.resetBtn:SetPoint("RIGHT", lb.selfResetBtn, "LEFT", -10, 0); lb.resetBtn:SetText("Reset DB"); lb.resetBtn:SetScript("OnClick", function() if UnitIsGroupLeader("player") then StaticPopup_Show("DTC_RESET_CONFIRM") end end)
    lb:Hide()
end
...
function DTC_RefreshLeaderboard()
    ...
    DTC_LeaderboardFrame.configBtn:SetShown(true) -- Config visible for all (permissions handled inside)
    ...
    if selExp == "ALL" then UIDropDownMenu_DisableDropDown(DTC_LeaderboardFrame.ddRaid); UIDropDownMenu_DisableDropDown(DTC_LeaderboardFrame.ddBoss)
    else UIDropDownMenu_EnableDropDown(DTC_LeaderboardFrame.ddRaid); if selRaid == "ALL" then UIDropDownMenu_DisableDropDown(DTC_LeaderboardFrame.ddBoss) else UIDropDownMenu_EnableDropDown(DTC_LeaderboardFrame.ddBoss) end end
...
function DTC_GetSortedData()
    ...
    if selTime == "ALL" then
        ...
        for _, h in ipairs(DTCRaidDB.history) do
            if h.d == today then
                local expMatch = (selExp == "ALL"); if not expMatch then local raids = STATIC_DATA[tonumber(selExp)] or {}; if raids[h.r] then expMatch = true end end
                local raidMatch = (selRaid == "ALL" or h.r == selRaid); local bossMatch = (selBoss == "ALL" or h.b == selBoss)
                if expMatch and raidMatch and bossMatch then rawData[h.w] = (rawData[h.w] or 0) + h.p end
            end
        end
    end
    local displayData = {}
    ...
    local macScore = displayData["Mac"]; local pinkScore = displayData["Pink"]
