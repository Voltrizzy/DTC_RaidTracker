...
-- Creates a dark-styled button with a thin border and gold hover effect.
local function CreateStyledButton(parent, width, height, text)
    ...
    btn:SetBackdropColor(0.08, 0.08, 0.09, 0.95)
    ...
    btn:HookScript("OnEnter", function(self)
        if self:IsEnabled() then
            self:SetBackdropBorderColor(0.85, 0.65, 0.1, 1)
            self:SetBackdropColor(0.13, 0.12, 0.04, 0.95)
        end
    end)
    ...
    function btn:Disable()
        origDisable(self)
        self:SetBackdropBorderColor(0.20, 0.20, 0.20, 0.5)
        self:SetBackdropColor(0.05, 0.05, 0.05, 0.80)
        self._label:SetTextColor(0.45, 0.45, 0.45, 1)
    end
...
-- Applies gold color to an OptionsSliderTemplate's title label.
local function StyleSliderLabel(sliderName)
    local t = _G[sliderName .. "Text"]
    if t then t:SetTextColor(1, 0.78, 0.1) end
end
...
function DTC.Config:Init()
    ...
    title:SetText("DTC Raid Tracker")
    title:SetTextColor(1, 0.78, 0.1)
    ...
    local function CreateTabButton(id, text, relativeTo)
        local t = CreateFrame("Button", nil, panel, "BackdropTemplate")
        t:SetID(id)
        t:SetSize(100, 22)
        t:SetBackdrop({
            bgFile   = "Interface\\Buttons\\WHITE8X8",
        ...
        t:SetBackdropColor(0.08, 0.08, 0.09, 0.9)
        t:SetBackdropBorderColor(0.22, 0.22, 0.22, 1)

        local label = t:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
        label:SetAllPoints()
        label:SetJustifyH("CENTER")
        label:SetJustifyV("MIDDLE")
        label:SetText(text)
        ...
        t:SetHighlightTexture(hl)

        -- SetSelected(true) = active tab (gold), SetSelected(false) = inactive
        function t:SetSelected(selected)
            if selected then
                self:SetBackdropColor(0.12, 0.10, 0.02, 0.95)
                self:SetBackdropBorderColor(0.85, 0.65, 0.10, 1)
                self._label:SetTextColor(1, 0.82, 0.10, 1)
            else
                self:SetBackdropColor(0.08, 0.08, 0.09, 0.9)
                self:SetBackdropBorderColor(0.22, 0.22, 0.22, 1)
                self._label:SetTextColor(0.85, 0.85, 0.85, 1)
            end
        end

        -- Keep Disable/Enable for compatibility with old SelectTab pattern
        function t:Disable() self:SetSelected(true) end
        function t:Enable()  self:SetSelected(false) end

        t:HookScript("OnEnter", function(self)
            -- Only highlight border if not already selected
            if self._label:GetTextColor() < 0.9 then
                self:SetBackdropBorderColor(0.55, 0.55, 0.55, 1)
            end
        end)
        t:HookScript("OnLeave", function(self)
            -- Restore correct border without re-running SetSelected
            if self._label:GetTextColor() < 0.9 then
                self:SetBackdropBorderColor(0.22, 0.22, 0.22, 1)
            end
        end)
        ...
        if id == 1 then
            ...
            t:SetPoint("LEFT", relativeTo, "RIGHT", 4, 0)
        end
...
function DTC.Config:BuildGeneralTab(frame)
    ...
    local btnVote = CreateStyledButton(box, 140, 24, DTC.L["Test Vote Window"])
    btnVote:SetPoint("TOPLEFT", 15, -40)
    btnVote:SetScript("OnClick", function() if DTC.Vote then DTC.Vote:StartSession("Test Boss", true) end end)

    local btnLB = CreateStyledButton(box, 140, 24, DTC.L["Test Leaderboard"])
    btnLB:SetPoint("LEFT", btnVote, "RIGHT", 10, 0)
    ...
    local btnHist = CreateStyledButton(box, 140, 24, DTC.L["Test History"])
    btnHist:SetPoint("LEFT", btnLB, "RIGHT", 10, 0)
    ...
    local btnBribe = CreateStyledButton(box, 140, 24, DTC.L["Test Incoming Bribe"])
    btnBribe:SetPoint("TOPLEFT", 15, -70)
    ...
    local btnProp = CreateStyledButton(box, 140, 24, DTC.L["Test Proposition"])
    btnProp:SetPoint("LEFT", btnBribe, "RIGHT", 10, 0)
    ...
    local btnLobby = CreateStyledButton(box, 140, 24, DTC.L["Test Lobbying"])
    btnLobby:SetPoint("LEFT", btnProp, "RIGHT", 10, 0)
    ...
    local btnDebts = CreateStyledButton(box, 140, 24, DTC.L["Test Debts"])
    btnDebts:SetPoint("TOPLEFT", 15, -100)
    ...
    local btnVer = CreateStyledButton(box, 140, 24, DTC.L["Version Check"])
    btnVer:SetPoint("TOPLEFT", 15, -130)
    ...
    local btnReset = CreateStyledButton(box, 140, 24, DTC.L["Reset to Defaults"])
    btnReset:SetPoint("LEFT", btnVer, "RIGHT", 10, 0)
    btnReset:SetScript("OnClick", function() StaticPopup_Show("DTC_RESET_SETTINGS_CONFIRM") end)

    local btnForceStart = CreateStyledButton(box, 140, 24, DTC.L["Force Start Session"])
    btnForceStart:SetPoint("TOPLEFT", 15, -160)
...
function DTC.Config:BuildNicknamesTab(frame)
    ...
    local btnSelectAll = CreateStyledButton(box, 100, 24, DTC.L["Select All"])
    btnSelectAll:SetPoint("BOTTOMLEFT", 15, 10)
    ...
    local btnDelSel = CreateStyledButton(box, 120, 24, DTC.L["Delete Selected"])
    btnDelSel:SetPoint("LEFT", btnSelectAll, "RIGHT", 10, 0)
    ...
    for _, guild in ipairs(sortedGuilds) do
        ...
        for _, name in ipairs(players) do
            ...
            if not row then
                ...
                row.DelBtn = CreateFrame("Button", nil, row, "BackdropTemplate")
                row.DelBtn:SetSize(20, 20)
                row.DelBtn:SetPoint("LEFT", row.EditBox, "RIGHT", 5, 0)
                row.DelBtn:SetBackdrop({ bgFile="Interface\\Buttons\\WHITE8X8", edgeFile="Interface\\Buttons\\WHITE8X8", edgeSize=1 })
                row.DelBtn:SetBackdropColor(0.08, 0.08, 0.09, 0.95)
                row.DelBtn:SetBackdropBorderColor(0.30, 0.30, 0.30, 1)
                local dLabel = row.DelBtn:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
                dLabel:SetAllPoints(); dLabel:SetJustifyH("CENTER"); dLabel:SetJustifyV("MIDDLE"); dLabel:SetText("X")
                row.DelBtn:HookScript("OnEnter", function(self) self:SetBackdropBorderColor(0.85, 0.65, 0.1, 1) end)
                row.DelBtn:HookScript("OnLeave", function(self) self:SetBackdropBorderColor(0.30, 0.30, 0.30, 1) end)
...
function DTC.Config:BuildHistoryTab(frame)
    ...
    local btnReset = CreateStyledButton(b1, 160, 24, DTC.L["Reset Local Data"])
    btnReset:SetPoint("TOPLEFT", 15, -40)
    btnReset:SetScript("OnClick", function() StaticPopup_Show("DTC_RESET_CONFIRM") end)

    local btnBribeHist = CreateStyledButton(b1, 140, 24, DTC.L["Open Bribe Ledger"])
    btnBribeHist:SetPoint("LEFT", btnReset, "RIGHT", 10, 0)
    ...
    local btnSync = CreateStyledButton(bSync, 120, 24, DTC.L["Push Data"])
    btnSync:SetPoint("LEFT", ebSync, "RIGHT", 10, 0)
    ...
    local btnPurge = CreateStyledButton(bPurge, 160, 24, DTC.L["Purge Matching"])
    btnPurge:SetPoint("TOPLEFT", 20, -75)
    btnPurge:SetScript("OnClick", function()
        StaticPopup_Show("DTC_PURGE_CONFIRM", nil, nil, pFilters)
    end)
end
...
function DTC.Config:BuildVotingTab(frame)
    ...
    s:SetMinMaxValues(30, 600); s:SetValueStep(10); s:SetObeyStepOnDrag(true); s:SetWidth(200)
    _G[s:GetName().."Text"]:SetText(DTC.L["Voting Window Duration"]); StyleSliderLabel(s:GetName())
    _G[s:GetName().."Low"]:SetText("30s"); _G[s:GetName().."High"]:SetText("10m")
    ...
    -- Votes Per Person Slider
    local sVotes = CreateFrame("Slider", "DTC_VotesPerPersonSlider", b1, "OptionsSliderTemplate")
    sVotes:SetPoint("LEFT", s, "RIGHT", 40, 0)
    sVotes:SetMinMaxValues(1, 10); sVotes:SetValueStep(1); sVotes:SetObeyStepOnDrag(true); sVotes:SetWidth(200)
    _G[sVotes:GetName().."Text"]:SetText(DTC.L["Votes Per Person"]); StyleSliderLabel(sVotes:GetName())
    _G[sVotes:GetName().."Low"]:SetText("1"); _G[sVotes:GetName().."High"]:SetText("10")
    ...
    local btnReset = CreateStyledButton(b1, 120, 22, DTC.L["Reset Defaults"])
    btnReset:SetPoint("TOPRIGHT", -20, -30)
    ...
    _G[sCount:GetName().."Text"]:SetText(DTC.L["Active Message Count"]); StyleSliderLabel(sCount:GetName())
    ...
    local btnTest = CreateStyledButton(content, 140, 20, DTC.L["Test Announcement"])
    btnTest:SetPoint("LEFT", sCount, "RIGHT", 20, 0)
    ...
    for i = 1, 10 do
        ...
        local lbl2 = row:CreateFontString(nil, "OVERLAY", "GameFontHighlight"); lbl2:SetPoint("LEFT", 0, 0); lbl2:SetWidth(20); lbl2:SetText(i..".")
        local eb = CreateFrame("EditBox", nil, row, "InputBoxTemplate"); eb:SetSize(480, 20); eb:SetPoint("LEFT", lbl2, "RIGHT", 5, 0); eb:SetAutoFocus(false)
    ...
    local function AddEdit(title, key, y, toggleKey)
        ...
        e:SetScript("OnShow",          function(self) self:SetText(DTCRaidDB.settings[key] or "") end)
        e:SetScript("OnEditFocusLost", function(self) DTCRaidDB.settings[key] = self:GetText() end)
...
function DTC.Config:BuildBribeTab(frame)
    ...
    local function AddSlider(title, key, minVal, maxVal, y, suffix)
        ...
        s:SetMinMaxValues(minVal, maxVal); s:SetValueStep(5); s:SetObeyStepOnDrag(true); s:SetWidth(200)

        local sText = _G[name.."Text"]
        local sLow  = _G[name.."Low"]
        local sHigh = _G[name.."High"]

        if sText then sText:SetText(title); sText:SetTextColor(1, 0.78, 0.1) end
        ...
        s:SetScript("OnShow", function(self)
            self.isSettingUp = true
            local defaults = { bribeTimer=90, propTimer=90, lobbyTimer=120, corruptionFee=10 }
            local val = DTCRaidDB.settings[key] or defaults[key] or minVal
            self:SetValue(val)
            ...
            if key == "corruptionFee" then
                if UnitIsGroupLeader("player") or not IsInRaid() then
                    self:Enable()
                    if sLow  then sLow:SetTextColor(1,1,1) end
                    if sHigh then sHigh:SetTextColor(1,1,1) end
                    if sText then sText:SetTextColor(1, 0.78, 0.1) end
                else
                    self:Disable()
                    if sLow  then sLow:SetTextColor(0.5,0.5,0.5) end
                    if sHigh then sHigh:SetTextColor(0.5,0.5,0.5) end
                    if sText then sText:SetTextColor(0.5,0.5,0.5) end
                end
            end
            self.isSettingUp = false
        end)
    ...
    local btnAnnounce = CreateStyledButton(b2, 160, 24, DTC.L["Announce Debts"])
    btnAnnounce:SetPoint("TOPLEFT", 20, -40)
